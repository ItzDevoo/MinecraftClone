...
namespace DevCraft
{
    ...
    { 
        ...
        void CheckInput()
        {
            ...
            else if (Util.KeyPressed(Keys.Space, currentKeyboardState, previousKeyboardState))
            {
                lastPressTime = gameTime.TotalGameTime.TotalMilliseconds;
                lastKeyPressed = Keys.Space;

                // Jump when on ground and not flying
                if (!Flying && Walking)
                {
                    Physics.Velocity.Y = 0.19f; // Improved jump velocity
                    Walking = false;
                }
            }

            // Double-tap W to toggle sprinting
            ...
            // Stop sprinting when W is released or when not moving forward
