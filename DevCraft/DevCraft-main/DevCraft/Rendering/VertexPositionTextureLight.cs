...
public struct VertexPositionTextureLight(Vector3 position, Vector2 textureCoordinate, float light) : IVertexType
{
    ...
    // Implicit conversion from the new vertex structure for compatibility
    public static implicit operator VertexPositionTextureLight(VertexPositionTextureLightColor vertex)
    {
        return new VertexPositionTextureLight(vertex.Position, vertex.TextureCoordinate, vertex.Light);
    }
}
