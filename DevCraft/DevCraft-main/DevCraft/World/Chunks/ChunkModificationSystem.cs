...
class ChunkModificationSystem(FilePersistenceService persistenceService,
    BlockMetadataProvider blockMetadata, LightSystem lightSystem, Action<Chunk> PostChunkForRemeshing)
{
    readonly FilePersistenceService persistenceService = persistenceService;
    ...
    void RemoveBlock(Chunk chunk, Vec3<byte> blockIndex)
    {
        ...
        persistenceService.AddDelta(chunk.Index, blockIndex, Block.Empty);
    }
    ...
    void AddBlock(Chunk chunk, Block block, Vec3<byte> blockIndex, Vector3 rayDirection)
    {
        ...
        persistenceService.AddDelta(chunk.Index, newBlockIndex, block);
    }
