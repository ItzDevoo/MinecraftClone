...
namespace DevCraft.World.Lighting;
...
public class LightSystem
{
    ...
    public static FacesData<LightValue> GetFacesLight(FacesState visibleFaces, int x, int y, int z, Chunk chunk)
    {
        ...
        // Calculate a unified light value for all faces to eliminate light seams
        // Use the maximum light value from all adjacent air blocks
        LightValue maxLight = new LightValue(0, 0);

        // Sample light from all six directions and use the maximum
        if (visibleFaces.ZPos)
        {
            LightValue light;
            if (z == Chunk.Last)
            {
                light = chunk.ZPos.GetLight(x, y, 0);
            }
            else
            {
                light = chunk.GetLight(x, y, z + 1);
            }
            lightValues.ZPos = light;
            if (light.SkyValue > maxLight.SkyValue) maxLight = new LightValue(light.SkyValue, maxLight.BlockValue);
            if (light.BlockValue > maxLight.BlockValue) maxLight = new LightValue(maxLight.SkyValue, light.BlockValue);
        }
        ...
        if (visibleFaces.ZNeg)
        {
            ...
            if (z == 0)
            {
                light = chunk.ZNeg.GetLight(x, y, Chunk.Last);
            }
            else
            {
                light = chunk.GetLight(x, y, z - 1);
        ...
        if (visibleFaces.YPos)
        {
            ...
            if (y == Chunk.Last)
            {
                light = chunk.YPos.GetLight(x, 0, z);
            }
            else
            {
                light = chunk.GetLight(x, y + 1, z);
            }
            lightValues.YPos = light;
            if (light.SkyValue > maxLight.SkyValue) maxLight = new LightValue(light.SkyValue, maxLight.BlockValue);
            if (light.BlockValue > maxLight.BlockValue) maxLight = new LightValue(maxLight.SkyValue, light.BlockValue);
        ...
        if (visibleFaces.YNeg)
        {
            ...
            if (y == 0)
            {
                light = chunk.YNeg.GetLight(x, Chunk.Last, z);
            }
            else
            {
                light = chunk.GetLight(x, y - 1, z);
        ...
        if (visibleFaces.XPos)
        {
            ...
            if (x == Chunk.Last)
            {
                light = chunk.XPos.GetLight(0, y, z);
            }
            else
            {
                light = chunk.GetLight(x + 1, y, z);
            }
            lightValues.XPos = light;
            if (light.SkyValue > maxLight.SkyValue) maxLight = new LightValue(light.SkyValue, maxLight.BlockValue);
            if (light.BlockValue > maxLight.BlockValue) maxLight = new LightValue(maxLight.SkyValue, light.BlockValue);
        ...
        if (visibleFaces.XNeg)
        {
            ...
            if (x == 0)
            {
                light = chunk.XNeg.GetLight(Chunk.Last, y, z);
            }
            else
            {
                light = chunk.GetLight(x - 1, y, z);
        ...
        // Use the maximum light value for all visible faces to eliminate seams
        if (visibleFaces.ZPos) lightValues.ZPos = maxLight;
        if (visibleFaces.ZNeg) lightValues.ZNeg = maxLight;
        if (visibleFaces.YPos) lightValues.YPos = maxLight;
        if (visibleFaces.YNeg) lightValues.YNeg = maxLight;
        if (visibleFaces.XPos) lightValues.XPos = maxLight;
        if (visibleFaces.XNeg) lightValues.XNeg = maxLight;
